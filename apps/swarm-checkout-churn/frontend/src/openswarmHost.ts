...
/**
 * Whether the host SDK can actually be reached.
 *
 * HOST is localhost:8324, which on a publicly deployed build resolves to the
 * VISITOR's own machine, not the operator's — so every helper here fails there
 * (and is blocked as mixed content besides). The injected ?token= is only present
 * when the page is rendered inside OpenSwarm, which makes it a reliable signal for
 * hiding operator-only controls instead of showing buttons that always error.
 */
export function isInsideOpenSwarm(): boolean {
  return hostToken().length > 0;
}
...
export interface WorkflowScheduleInput {
  ...
  /** Sun=0..Sat=6, used when repeat_unit is 'week'. */
  on_days?: number[];
  hour?: number;
  minute?: number;
  timezone?: string;
...
/** Create a saved workflow in the user's Workflows hub, as a real card they can
 * inspect, pause, edit, or delete. */
export async function createWorkflow(input: CreateWorkflowInput): Promise<WorkflowSummary> {
  return hostFetch<WorkflowSummary>('/api/workflows/create', {
    method: 'POST',
    body: JSON.stringify({
      title: input.title,
      ...
      schedule: input.schedule ? { timezone: 'local', ...input.schedule } : null,
    }),
  });
}
