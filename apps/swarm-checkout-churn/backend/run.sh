...
if [[ -d "$VENV_DIR" && -f "$SENTINEL" ]]; then
    ...
    # A bare `-d "$VENV_DIR"` test treats a half-built venv as usable. The
    # bundled interpreter ships without pip AND without ensurepip, so
    # `python -m venv` leaves behind a directory with symlinks and an empty
    # site-packages, then every later run skips creation and dies on the
    # pip step forever. Probe for a working interpreter instead.
    if [[ ! -x "$VENV_PY" ]] || ! "$VENV_PY" -c "import sys" &>/dev/null; then
        echo "Creating virtual environment..."
        rm -rf "$VENV_DIR"
        # --system-site-packages inherits the host's preinstalled deps
        # (fastapi, uvicorn, typeguard, swarm-debug). ensurepip still fails
        # on the bundled Python, so the exit code is not trustworthy here;
        # the interpreter check below is what actually gates success.
        "$PYTHON" -m venv --system-site-packages "$VENV_DIR" || true
        if [[ ! -x "$VENV_PY" ]] || ! "$VENV_PY" -c "import sys" &>/dev/null; then
    ...
    # Only reach for pip when something is actually missing. On the bundled
    # Python every dependency is already importable and pip does not exist,
    # so installing would fail on a venv that is in fact complete.
    if "$VENV_PY" -c "import fastapi, uvicorn, typeguard, swarm_debug" &>/dev/null; then
        echo "Dependencies satisfied from system site-packages — skipping pip install."
    else
...
# With no pip install there is no console script in the venv, so fall back to
# the module entrypoint inherited from system site-packages.
if [[ -x "$SWARM_DEBUG_BIN" ]]; then
    ( cd "$BACKEND_DIR_ABSPATH/.." && "$SWARM_DEBUG_BIN" toggle on --all >/dev/null 2>&1 ) || true
else
    ( cd "$BACKEND_DIR_ABSPATH/.." && "$VENV_PY" -m swarm_debug toggle on --all >/dev/null 2>&1 ) || true
fi
