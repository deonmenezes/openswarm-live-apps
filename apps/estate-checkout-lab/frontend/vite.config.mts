...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      tailwindcss(),
      // PUBLIC_BUILD drops the inspector route from the bundle. /sessions lists every
      // visitor's captured fields and every order's buyer name + email; that is fine
      // inside OpenSwarm (one operator, local backend) and a data leak on a public
      // host, where the subjects of the telemetry are the visitors themselves.
      Pages({
        dirs: 'src/pages',
        extensions: ['tsx'],
        exclude: process.env.PUBLIC_BUILD ? ['**/sessions.tsx'] : [],
      }),
